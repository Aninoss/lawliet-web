...
public class StripeManager {
    ...
    public static String generateCheckoutSession(SubDuration duration, SubLevel level, SubCurrency currency, long discordId, String discordTag, String discordAvatar, int quantity) throws StripeException {
        ...
        SessionCreateParams.Builder paramsBuilder = new SessionCreateParams.Builder()
                ...
                .putMetadata("unlock_servers", String.valueOf(level == SubLevel.PRO))
                .putMetadata("quantity", String.valueOf(quantity))
                .putMetadata("tier", WordUtils.capitalizeFully(level.name() + " " + duration.name()))
                ...
                .addLineItem(new SessionCreateParams.LineItem.Builder()
                        ...
                        .setPrice(StripeManager.getPriceId(duration, level, currency))
                        .build()
                );

        Optional<Customer> customerOpt = StripeManager.retrieveCustomer(discordId, currency);
    ...
    public static String getPriceId(SubDuration duration, SubLevel level, SubCurrency currency) {
        String levelCurrency = level.name() + "_" + currency.name();
        if (duration == SubDuration.MONTHLY) {
            switch (levelCurrency) {
                case "BASIC_USD":
